...
class Ticketcarib{
    ...
    public static function listar(){
        $sql = "SELECT T1.idticketcaribe, T4.nombre AS nombrech, T1.fecha, T1.codigo, T2.nombre, T3.nombre AS nombrec, T1.montop, T1.condicion FROM ticketcaribe AS T1 LEFT JOIN cliente AS T2 ON T2.idcliente = T1.idcliente LEFT JOIN centro AS T3 ON T3.idcentro=T1.idcentro LEFT JOIN chofer AS T4 ON T4.idchofer = T1.idchofer WHERE estado=0";
        return Consulta($sql);
    }
