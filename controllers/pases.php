...
switch ($_GET["op"]){
    case 'guardaryeditar':
		if ($tipo_pase != "" || $startDate != "" || $endDate != "" ){
            ...
            $pdf->SetXY($X+90,$Y+20);
            $pdf->Cell(100,5,'Puerto La Cruz - '.$pdf->obFecha($startDate),0,0,'R');
            $pdf->SetXY($X+90,$Y+25);
            ...
            $pdf->SetXY($X+90,$Y+40);
            $pdf->Cell(100,5,utf8_decode('Cc:  Sr. Juan Iwimas (Oficial de P.B.I.P).'),0,0,'R');
            $pdf->SetXY($X+20,$Y+55);
            ...
            //SE DEBE CREAR ARRAY CON LA TABLA DE CHOFERES Y LUEGO ENVIARLA
            
            $pases->tablaChoferes($header, $choferes);
